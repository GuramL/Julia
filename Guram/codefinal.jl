#############################
# myerson8_distributed.jl
# Distributed simulation with correct p* and optional per-period price trails
#############################
...
@everywhere function pstar_true(mu::Float64, sigma::Float64)
    ...
    # Robust bracketing around μ; the sign change is near μ + O(1)
    lo = mu
    hi = mu + 50.0                         # wide but finite bracket
    # Ensure sign change; expand if needed a few times
    vlo = g(lo); vhi = g(hi)
    expand = 0
...
@everywhere function simulate_sigma(j::Int, mu::Float64, sigmamin::Float64, sigmaincre::Float64,
    ...
    for t in 1:T
        ...
        eprice = -β1 / (2*β2)                 # belief-optimal (myopic) price at t
        p      = eprice + e[t]
    ...
    # --- Cole–Roughgarden sampling benchmark ---
...
df = DataFrame(rows)
CSV.write("myerson8.csv", df)
println("Saved results to myerson8.csv")
